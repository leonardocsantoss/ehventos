...
def browse(request):
    ...
    query['dir'] = get_path(query.get('dir', str(request.user.username)))
...
def mkdir(request):
    ...
    # QUERY / PATH CHECK
    query = request.GET
    path = get_path(query.get('dir', str(request.user.username)))
    if path is None:
        msg = _('The requested Folder does not exist.')
        request.user.message_set.create(message=msg)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            server_path = os.path.join(abs_path, remove_sc(form.cleaned_data['dir_name']))
            try:
                # PRE CREATE SIGNAL
                filebrowser_pre_createdir.send(sender=request, path=path, dirname=remove_sc(form.cleaned_data['dir_name']))
                # CREATE FOLDER
                os.mkdir(server_path)
                os.chmod(server_path, 0775)
                # POST CREATE SIGNAL
                filebrowser_post_createdir.send(sender=request, path=path, dirname=remove_sc(form.cleaned_data['dir_name']))
                # MESSAGE & REDIRECT
                msg = _('The Folder %s was successfully created.') % (remove_sc(form.cleaned_data['dir_name']))
                request.user.message_set.create(message=msg)
...
def delete(request):
    ...
    query = request.GET
    path = get_path(query.get('dir', str(request.user.username)))
    filename = get_file(query.get('dir', str(request.user.username)), query.get('filename', ''))
